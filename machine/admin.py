...
class MachineAdmin(admin.ModelAdmin):
    ...
    def MachineOutputLines( self, request ):
        machine = Machine.objects.get( Machine_ID=request.pk )
        if machine.is_data_imported:
            return mark_safe( f"<a href='/admin/Machine/{machine.Machine_ID}/DataOutputLines'>Machine_{machine.Machine_ID}_DataOutputLines</a>" )
        else:
            return ''
...
@login_required
def AdminDataOutputLines( request, Machine_ID ):
    machine = get_object_or_404( Machine, pk=Machine_ID )
    context = { }
    model = machine.get_machine_data_output_lines_model()
    columns = machine.get_machine_data_output_lines_columns( include_predefined=True )
    context.update( locals() )
    return render( request, 'admin/AdminDataOutputLines.html', context )


@method_decorator(login_required, name='dispatch')
class MachineDatatableOutputAjax(datatable.DTView):
    """ Return Output data. Using with jquery.datatables """
    def get(self, request, Machine_ID):
        # return JSON
        machine = get_object_or_404( Machine, pk=Machine_ID )

        self.model = machine.get_machine_data_output_lines_model()
        self.columns = machine.get_machine_data_output_lines_columns( include_predefined=True )
        self.order_columns = self.columns
        return super().get(request)
